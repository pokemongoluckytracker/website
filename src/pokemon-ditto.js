...
async function getData() {
  await fetch("https://pokemon-go1.p.rapidapi.com/possible_ditto_pokemon.json", {
    "method": "GET",
    "headers": {
  ...
  .then(response => {
    fs.writeFile("src/pokemon-ditto.json", JSON.stringify(response), (err) => {
      if (err) console.log(err);
      console.log("Successfully written to ditto File.");
    });
  })
  .catch(err => {
    console.log(err);
